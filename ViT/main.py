...
from model import ViT,ViT_tiny
...
def main():
    ...
    save_dir = os.path.join("./logs", args.source[0].upper() + ' -> ' + args.target[0].upper() + datetime.now().strftime('\n%m%d\n%H:%M'))
    ...
    #transformer = ViT().cuda()
    transformer = ViT_tiny().cuda()
    ...
    for epoch in range(args.epochs):
        ...
        utils.evaluate(transformer, tgt_test_loader, _print)
